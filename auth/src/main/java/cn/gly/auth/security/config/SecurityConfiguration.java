package cn.gly.auth.security.config;


import cn.gly.auth.security.filter.AuthenticationFailEntryPoint;
import cn.gly.auth.security.filter.access.JwtAuthenticationTokenFilter;
import cn.gly.auth.security.filter.authentication.CreateAuthenticationTokenFilter;
import cn.gly.auth.security.filter.authentication.OptionsRequestFilter;
import cn.gly.auth.security.service.AuthenticationUserService;
import cn.gly.auth.security.util.JwtTokenUtil;
...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    public void configure(WebSecurity web) {
        ...
        ignore.asMap().entrySet().stream()
                ...
                .forEach(entry ->
                        web.ignoring().antMatchers(entry.getKey(), entry.getValue().toArray(new String[entry.getValue().size()])));
    }
